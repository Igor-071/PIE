...
// Note: Using any types here since schema types are in the parent project
// In production, consider sharing types via a common package
 ...
 */
export function validatePrd(prd: any): ValidationResult {
  ...
  if (prd.screens && prd.screens.length > 0) {
    prd.screens.forEach((screen: any, index: number) => {
  ...
  if (prd.targetAudience && prd.targetAudience.length > 0) {
    prd.targetAudience.forEach((audience: any, index: number) => {
  ...
  if (prd.api && prd.api.length > 0) {
    prd.api.forEach((endpoint: any, index: number) => {
