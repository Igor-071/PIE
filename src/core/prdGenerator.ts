...
function generateRoutePath(screenName: string): string {
...
function generateEventDescription(event: any): string {
  ...
  if (handler.includes('login')) return 'User submits login form';
  if (handler.includes('signout') || handler.includes('logout')) return 'User clicks sign out button';
  if (handler.includes('navigate')) return 'User clicks navigation link';
  if (handler.includes('upload')) return 'User uploads a file';
  if (handler.includes('delete')) return 'User deletes an item';
  if (handler.includes('create') || handler.includes('add')) return 'User creates a new item';
  if (handler.includes('edit') || handler.includes('update')) return 'User edits an existing item';
  return 'User interaction event';
}

function generateDeliveryTimelineSection(timeline: any): string {
...
function generateLaunchPlanSection(launchPlan: any): string {
  ...
  if (launchPlan.launchStrategy) {
    ...
    if (launchPlan.launchStrategy.phases) {
      launchPlan.launchStrategy.phases.forEach((phase: any, index: number) => {
  ...
  if (launchPlan.successCriteria) {
    ...
    Object.entries(launchPlan.successCriteria).forEach(([period, criteria]: [string, any]) => {
...
function generateStakeholdersRaciSection(stakeholders: any): string {
  ...
  if (stakeholders.stakeholders && stakeholders.stakeholders.length > 0) {
    ...
    markdown += `|-------------|------|-----------|----------|------------------|\n`;
    stakeholders.stakeholders.forEach((sh: any) => {
  ...
  if (stakeholders.raciChart && stakeholders.raciChart.length > 0) {
    ...
    markdown += `|----------|-----------------|-----------------|-------------|------------------|--------|----|-----------|\n`;
    stakeholders.raciChart.forEach((activity: any) => {
...
function generateDesignRequirementsSection(design: any): string {
  ...
  if (design.designSystem) {
    ...
    if (design.designSystem.designTokens) {
      ...
      Object.entries(design.designSystem.designTokens).forEach(([key, value]: [string, any]) => {
        markdown += `- ${key}: ${value}\n`;
      });
      markdown += `\n`;
    }
...
function generateEnhancedDataModelsSection(dataModel: any): string {
  let markdown = `## 23. Data Models\n\n`;
  markdown += `### Entity Relationship Overview\n\n`;
  if (dataModel.entities) {
    Object.entries(dataModel.entities).forEach(([entityName, entity]: [string, any]) => {
...
function generateTestingStrategySection(testing: any): string {
  ...
  if (testing.testingTypes && testing.testingTypes.length > 0) {
    ...
    testing.testingTypes.forEach((type: any) => {
      markdown += `#### ${type.name}\n\n`;
      if (type.framework) markdown += `**Framework:** ${type.framework}\n\n`;
      if (type.scope && type.scope.length > 0) {
        markdown += `**Scope:**\n`;
        type.scope.forEach((item: string) => {
          markdown += `- ${item}\n`;
        });
        markdown += `\n`;
      }
...
function generateDeploymentStrategySection(deployment: any): string {
  ...
  if (deployment.deploymentProcess) {
    ...
    if (deployment.deploymentProcess.deploymentSteps) {
      ...
      deployment.deploymentProcess.deploymentSteps.forEach((step: string, index: number) => {
...
function generateAnalyticsMonitoringSection(analytics: any): string {
  ...
  if (analytics.keyMetrics) {
    ...
    if (analytics.keyMetrics.userMetrics && analytics.keyMetrics.userMetrics.length > 0) {
      ...
      analytics.keyMetrics.userMetrics.forEach((metric: string) => {
        markdown += `- ${metric}\n`;
      });
      markdown += `\n`;
    }
  ...
  if (analytics.alertingThresholds) {
    ...
    if (analytics.alertingThresholds.critical && analytics.alertingThresholds.critical.length > 0) {
      ...
      analytics.alertingThresholds.critical.forEach((alert: any) => {
