 ...
 */
export async function detectApiEndpoints(
  repoPath: string,
  allFiles: string[]
): Promise<ApiEndpoint[]> {
 ...
 */
function extractApiCalls(content: string, filePath: string): ApiEndpoint[] {
  const endpoints: ApiEndpoint[] = [];
  ...
  const useQueryPattern =
    /use(?:Query|Mutation)\s*\([^)]*['"`]([^'"`]+)['"`]/gi;
  ...
  const apiClientPattern =
    /(?:api|apiClient|client)\.\w+\.(get|post|put|delete|patch)\s*\(\s*['"`]([^'"`]+)['"`]/gi;
