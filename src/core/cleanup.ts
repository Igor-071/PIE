 ...
 */
export async function cleanupDirectory(dirPath: string, ignoreErrors: boolean = true): Promise<void> {
  try {
 ...
 * @param ignoreErrors - If true, errors during cleanup are ignored
 */
export async function cleanupDirectories(dirPaths: string[], ignoreErrors: boolean = true): Promise<void> {
  await Promise.allSettled(
    dirPaths.map((dirPath) => cleanupDirectory(dirPath, ignoreErrors))
  );
}
 ...
 */
export async function cleanupOldDirectories(
  baseDir: string,
  maxAgeMs: number = 24 * 60 * 60 * 1000,
  pattern: RegExp = /^repo-\d+$/
): Promise<void> {
  try {
