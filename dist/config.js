...
function validateApiKey(apiKey) {
    ...
    if (!apiKey.startsWith("sk-")) {
        throw new Error('OPENAI_API_KEY appears to be invalid. OpenAI API keys typically start with "sk-". ' +
            "Please check your .env file or environment variables.");
    }
    // Basic length check (OpenAI keys are usually 51+ characters)
    if (apiKey.length < 20) {
        throw new Error("OPENAI_API_KEY appears to be too short. Please verify your API key is correct.");
    }
}
/**
...
function validateModel(model) {
    ...
    if (!validModels.includes(model)) {
        console.warn(`Warning: Model "${model}" is not in the list of known models. ` +
