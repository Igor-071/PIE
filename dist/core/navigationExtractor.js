 ...
 */
export async function extractNavigation(repoPath, screens) {
    const navigation = [];
    ...
    try {
        const routerFiles = screens.filter((s) => s.includes("routes") ||
            s.includes("router") ||
            s.includes("App.tsx") ||
            s.includes("App.jsx"));
    ...
    const uniqueNav = navigation.filter((item, index, self) => index === self.findIndex((t) => t.path === item.path));
    return uniqueNav;
}
/**
 * Extracts navigation from Next.js file-based routing
 */
function extractNextjsFileRoutes(screens) {
    const routes = [];
    const pageFiles = screens.filter((s) => s.includes("/pages/") || s.includes("/app/"));
 ...
 */
function parseNavigationFromCode(content) {
    const navigation = [];
    ...
    const navLinkPattern = /<NavLink\s+to=["']([^"']+)["'][^>]*>([^<]+)<\/NavLink>/gi;
    ...
    const navArrayPattern = /(?:const|let|var)\s+\w+\s*=\s*\[\s*\{[^}]*path:\s*["']([^"']+)["'][^}]*label:\s*["']([^"']+)["'][^}]*\}/gi;
 ...
 */
function parseReactRouterRoutes(content) {
    const routes = [];
...
//# sourceMappingURL=navigationExtractor.js.map