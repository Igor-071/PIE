...
async function collectDocsFromDirectories(repoPath, documents, dirNames) {
    const MAX_FILES = 30;
...
async function collectContractEvidence(repoPath) {
    const MAX_FILES = 12;
    ...
    async function traverse(dirPath, depth) {
        ...
        for (const entry of entries) {
            ...
            const looksLikeContract = name.includes("openapi") ||
 ...
 */
function buildPackageMetadataSummary(packageJson) {
    let summary = "";
    if (packageJson.name) {
    ...
    if (packageJson.dependencies) {
        ...
        const frameworks = deps.filter((d) => ["react", "next", "vue", "angular", "svelte", "express", "fastify", "koa"].includes(d));
        const uiLibs = deps.filter((d) => ["@radix-ui", "@mui", "antd", "chakra-ui", "tailwindcss"].some((lib) => d.includes(lib)));
        const stateManagement = deps.filter((d) => ["redux", "zustand", "recoil", "mobx", "jotai", "@tanstack/react-query"].includes(d));
        const databases = deps.filter((d) => ["prisma", "mongoose", "typeorm", "sequelize", "@supabase"].some((lib) => d.includes(lib)));
        const auth = deps.filter((d) => ["next-auth", "@auth", "passport", "clerk", "supabase", "firebase"].some((lib) => d.includes(lib)));
        if (frameworks.length > 0)
            summary += `  - Frameworks: ${frameworks.join(", ")}\n`;
        if (uiLibs.length > 0)
            summary += `  - UI Libraries: ${uiLibs.join(", ")}\n`;
        if (stateManagement.length > 0)
            summary += `  - State Management: ${stateManagement.join(", ")}\n`;
        if (databases.length > 0)
            summary += `  - Database/ORM: ${databases.join(", ")}\n`;
        if (auth.length > 0)
            summary += `  - Authentication: ${auth.join(", ")}\n`;
        summary += "\n";
    }
...
function buildCodeSummary(tier1Data, minimal = false) {
    ...
    if (tier1Data.screens && tier1Data.screens.length > 0) {
        ...
        for (const screen of tier1Data.screens) {
            ...
            let feature = "General";
            if (screen.path.includes("/patient"))
                feature = "Patient Management";
            else if (screen.path.includes("/admin"))
                feature = "Administration";
            else if (screen.path.includes("/auth") || screen.path.includes("/login"))
                feature = "Authentication";
            else if (screen.path.includes("/dashboard"))
                feature = "Dashboard";
            else if (screen.path.includes("/setting"))
                feature = "Settings";
            else if (screen.path.includes("/report"))
                feature = "Reporting";
            else if (screen.path.includes("/inventory") || screen.path.includes("/stock"))
                feature = "Inventory";
            else if (screen.path.includes("/schedule") || screen.path.includes("/calendar") || screen.path.includes("/appointment"))
                feature = "Scheduling";
            else if (screen.name.toLowerCase().includes("patient"))
                feature = "Patient Management";
            else if (screen.name.toLowerCase().includes("inventory"))
                feature = "Inventory";
            else if (screen.name.toLowerCase().includes("schedule") || screen.name.toLowerCase().includes("appointment"))
                feature = "Scheduling";
    ...
    if (tier1Data.dataModels && tier1Data.dataModels.length > 0) {
        ...
        const modelNames = tier1Data.dataModels.map((m) => m.name);
    ...
    if (tier1Data.apiEndpoints && tier1Data.apiEndpoints.length > 0) {
        ...
        for (const [method, paths] of Object.entries(groupedByMethod)) {
            ...
            if (paths.length > 5)
                summary += `, ...and ${paths.length - 5} more`;
            summary += "\n";
        }
        summary += "\n";
    }
    ...
    if (tier1Data.statePatterns && tier1Data.statePatterns.length > 0) {
        ...
        const stateTypes = [...new Set(tier1Data.statePatterns.map((s) => s.type))];
 ...
 */
async function collectConfigFiles(repoPath) {
    const documents = [];
 ...
 */
function analyzeConfigContent(configName, content) {
    const insights = [];
 ...
 */
async function collectTestFiles(repoPath) {
    const documents = [];
 ...
 */
async function findTestFilesManually(repoPath) {
    const testFiles = [];
 ...
 */
async function analyzeComponents(repoPath, tier1Data) {
    if (!tier1Data?.screens || tier1Data.screens.length === 0) {
        return null;
    }
 ...
 */
async function analyzeCodePatterns(repoPath, tier1Data) {
    const patterns = {
        auth: [],
        dataFlow: [],
        errorHandling: [],
    ...
    if (tier1Data) {
        ...
        if (tier1Data.apiEndpoints) {
            tier1Data.apiEndpoints.forEach((endpoint) => {
        ...
        if (tier1Data.screens) {
            const roleScreens = tier1Data.screens.filter((s) => s.name?.toLowerCase().includes("admin") ||
            ...
            if (roleScreens.length > 0) {
                ...
                patterns.rbac.push(...roleScreens.slice(0, 10).map((s) => `- ${s.name}`));
            }
        }
        // Check state management patterns
        if (tier1Data.statePatterns) {
            tier1Data.statePatterns.forEach((pattern) => {
