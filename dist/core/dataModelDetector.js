 ...
 */
export async function detectDataModels(repoPath, allFiles) {
    const dataModel = {};
    // Find potential model files
    const modelFiles = allFiles.filter((f) => (f.endsWith(".ts") || f.endsWith(".d.ts")) &&
...
function extractZodSchemas(content) {
    ...
    const zodPattern = /(?:export\s+)?const\s+(\w+(?:Schema)?)\s*=\s*z\.object\s*\(\s*\{([^}]+)\}/g;
    let match;
...
//# sourceMappingURL=dataModelDetector.js.map